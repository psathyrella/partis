...
.createModel_DDexp <- function(tree){
        ...
        aAGamma <- function(i, params){
            ...
            matrixGamma <- function(t) return((exp(params[2])*exp((params[3]/2)*length(vectorU)))*diag(vectorU))
        ...
        model <- new(Class="PhenotypicADiag", name="DDexp", period=periodizing$periods, aAGamma=aAGamma, numbersCopy=eventEndOfPeriods$copy, numbersPaste=eventEndOfPeriods$paste, initialCondition=initialCondition, paramsNames=paramsNames, constraints=constraints, params0=params0, tipLabels=eventEndOfPeriods$labeling, comment=comment)
