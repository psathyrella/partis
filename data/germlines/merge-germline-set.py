...
def main():
    ...
    parser.add_argument('--expected', required=True, choices=['augmentation', 'replacement'],
                        help="where the new set comes from. NOTE this does *not* control whether genes get dropped: the merge is ALWAYS a union either way (old genes missing from the new set are always kept), --expected only sets which name/seq wins on a conflict. 'augmentation' = genes from a DIFFERENT source (e.g. engelbrecht/rodriguez on top of imgt), keep our old names; 'replacement' = a newer version of the SAME source (e.g. a fresh ogrdb pull), take the new names, and warn (loudly) about old genes missing from it, since for a re-download those are either real upstream removals or a botched download -- but either way they're kept, so removing them is a separate manual decision")
