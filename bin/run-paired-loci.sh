...
methods=simu:cache-parameters:partition:write-fake-paired-annotations:iqtree:gctree:gctree-base:tree-perf  # coar stuff cache-parameters:partition
...
echo $bin --label gct-valid --version v0 --n-replicates 2 --obs-times-list 50,75,90,100 --n-sim-seqs-per-generation-list 5,5,5,65 --n-sim-events-list 50 --carry-cap-list 500:1000:1500 --simu-extra-args=\"--context-depend 1\" --bcr-phylo --perf-metrics coar:rf --calc-antns --inference-extra-args=--no-indels --plot-metrics tree-perf $common
