...
echo $bin --label bcr-phylo-antn --version v0   --n-replicates 2 --obs-times-list 50:150:300 --n-sim-seqs-per-generation-list 15:45 --context-depend-list 1 --bcr-phylo --antn-perf --perf-metrics naive-hdist $common
...
echo $bin --label lcdr3 --version one-j --n-replicates 2 --n-sim-events-list 500 --single-light-locus igk --base-outdir /fh/fast/matsen_e/dralph/partis/paired-loci --allowed-cdr3-lengths ighM18-19:ighM21-22:ighM24-25 --simu-extra-args=\"--only-genes=IGHJ4*01:IGKJ1*01 --n-genes-per-region ::1\" $common
exit 0
