...
methods=simu:cache-parameters:partition:write-fake-paired-annotations:replay-plot:iqtree:raxml:gctree:gctree-base:gctree-mut-mult:tree-perf  # coar stuff cache-parameters:partition
...
echo $bin --label gct-valid --version v4 --n-replicates 2 --obs-times-list 10:15:20:30:40:50:100:150 --n-sim-events-list 70 --carry-cap-list 1000 --simu-type bcr-phylo --perf-metrics coar:rf:mrca --calc-antns --inference-extra-args=\"--no-indels --simultaneous-true-clonal-seqs\" --plot-metrics tree-perf --final-plot-xvar obs-times --final-plot-xvar obs-times $simu_extra $common  #  --pvks-to-plot "N=60; 100"
