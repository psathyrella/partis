...
echo $bin --label test-coar --version v1 --n-replicates 2 --obs-times-list 50 --n-sim-seqs-per-generation-list 15 --bcr-phylo --perf-metrics naive-hdist --calc-antns --inference-extra-args=--no-indels --plot-metrics iqtree-coar $common # need also: iqtree:write-fake-paired-annotations:coar
