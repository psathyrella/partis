...
echo $bin --label test-coar --version v3 --n-replicates 2 --obs-times-list 50,75,90,100 --n-sim-seqs-per-generation-list 5,5,5,50 --n-sim-events-list 50 --simu-extra-args=\"--carry-cap 500 --context-depend 1\" --bcr-phylo --perf-metrics coar:rf --calc-antns --inference-extra-args=--no-indels --plot-metrics tree-perf $common
