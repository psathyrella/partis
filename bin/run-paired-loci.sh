...
common="--n-sub-procs 30 --n-max-procs 5 --single-light-locus igk --base-outdir /fh/fast/matsen_e/dralph/partis/paired-loci $astr --dry" # /fh/local/dralph
...
# echo $bin --label bcr-phylo-antn --version v0   --n-replicates 2 --obs-times-list 50:150:300 --n-sim-seqs-per-generation-list 15:45 --context-depend-list 1 --bcr-phylo --dont-observe-common-ancestors --antn-perf --perf-metrics naive-hdist $common
simu_extra="--simu-extra-args=\"--target-distance 5 --aa-paratope-positions i=0:15 --aa-struct-positions i=20:len --dont-mutate-struct-positions\""
# --simu-extra-args=\"--context-depend 1\"
echo $bin --label gct-valid --version v2 --n-replicates 2 --obs-times-list 20:50 --n-sim-seqs-per-generation-list 75 --n-sim-events-list 50 --carry-cap-list 1000 --leaf-sampling-scheme affinity-biased:high-affinity --bcr-phylo --perf-metrics coar:rf --calc-antns --inference-extra-args=\"--no-indels --simultaneous-true-clonal-seqs\" --plot-metrics tree-perf $simu_extra $common
