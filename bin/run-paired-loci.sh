...
simu_extra="--simu-extra-args=\"--target-distance 5 --aa-paratope-positions N=15 --aa-struct-positions N=210 --dont-mutate-struct-positions --context-depend 1\""
echo $bin --label gct-valid --version v4-test-v0 --n-replicates 2 --obs-times-list 20:50 --n-sim-seqs-per-generation-list 75 --n-sim-events-list 50 --carry-cap-list 1000 --leaf-sampling-scheme affinity-biased:high-affinity --bcr-phylo --perf-metrics coar:rf --calc-antns --inference-extra-args=\"--no-indels --simultaneous-true-clonal-seqs\" --plot-metrics tree-perf $simu_extra $common
