...
methods=simu:cache-parameters:partition:write-fake-paired-annotations:iqtree:iqtree-coar:gctree:gctree-coar:gctree-base:gctree-base-coar  # coar stuff cache-parameters:partition
...
echo $bin --label test-coar --version v1 --n-replicates 2 --obs-times-list 50 --n-sim-seqs-per-generation-list 15 --bcr-phylo --perf-metrics coar --calc-antns --inference-extra-args=--no-indels --plot-metrics gctree-coar:gctree-base-coar:iqtree-coar $common # need also: iqtree:write-fake-paired-annotations:coar
