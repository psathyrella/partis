...
#     # sleep 0.5s
# =======
# loraxis_args="--no-slurm --n-max-procs 8 --baseworkdir /tmp/$USER"

# for istartstop in $istartstoplist; do  # see code below to generate these
#     # ./bin/compare-partition-methods.py --actions naive-hamming-partition:partition:run-viterbi:vsearch-partition --istartstop $istartstop $leaf_mut_hum $xtra &
#     # ./bin/compare-partition-methods.py --actions seed-partition --istartstop $istartstop $leaf_mut_hum $xtra &  #  --no-slurm
#     ./bin/compare-partition-methods.py --actions cache-parameters --istartstop $istartstop $leaf_mut_hum $xtra $loraxis_args #--dry-run
#     ./bin/compare-partition-methods.py --actions partition --istartstop $istartstop $leaf_mut_hum $xtra $loraxis_args # --dry-run
#     # sleep 1h
# >>>>>>> Stashed changes
