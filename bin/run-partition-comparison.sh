...
# # ./bin/compare-partition-methods.py --actions run-viterbi $leaf_mut_hum --box --n-to-partition 3000 &
# # ./bin/compare-partition-methods.py --actions write-plots --expected-methods vollmers-0.9:partition:naive-hamming-partition:vsearch-partition $leaf_mut_hum --box --n-to-partition 3000 --no-similarity-matrices &
# # ./bin/compare-partition-methods.py --actions write-plots --expected-methods vollmers-0.9 $leaf_mut_hum --box --n-to-partition 3000 &
# exit 0
echo "run the mimic samples"
# ----------------------------------------------------------------------------------------
...
for isub in 0 1 2; do
    ...
    ./bin/compare-partition-methods.py --actions run-mixcr --subset $isub --n-subsets 10 $leaf_mut_hum &  # --overwrite &
    # sleep 60
...
# ./bin/compare-partition-methods.py --actions compare-subsets --plot-mean-of-subsets --n-subsets 3 $leaf_mut_hum --expected-methods vollmers-0.9:changeo:partition:naive-hamming-partition:vsearch-partition
...
#     # ./bin/compare-partition-methods.py --actions vsearch-partition:naive-hamming-partition:partition --istartstop $istartstop $leaf_mut_hum &  # run-viterbi: --count-distances
#     # sleep 60
