...
label=cells-per-drop-$cpd # pair-plots-v0 #fix-bad-merge-v2 #2 # pairclean-v2  # re-refactor-v0 #refactor-v4  #paired-clustering-output-v2
# NOTE use tmp.sh for refactor-v{3,4}

nprocs=10
common="--n-sim-events 100 --n-leaves 10 --n-procs $nprocs --no-per-base-mutation --allowed-cdr3-lengths 30:33:36:42:45:48 --mutation-multiplier 1" # --debug 1" #  --constant-number-of-leaves
...
# ./bin/partis simulate --paired-loci --seed 1 --parameter-dir $in_param_dir --paired-outdir $outdir/simu $common --mean-cells-per-droplet $cpd >$outdir/simu.log
