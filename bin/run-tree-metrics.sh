...
# for mm in aa-lbi aa-lbr shm delta-lbi cons-dist-aa cons-dist-nuc; do
common="--actions get-tree-metrics --only-csv-plots --n-max-procs 25 $dtr_args" #  --metric-method $mm# --no-tree-plots --slurm
# common="--actions plot --plot-metrics aa-lbi:aa-lbr"  # cons-dist-nuc:cons-dist-aa"
...
# common="--actions combine-plots --plot-metrics shm:delta-lbi:lbi:lbr:cons-dist-aa:cons-dist-nuc:aa-lbi:aa-lbr --combo-extra-str with-aa-lb" # --pvks-to-plot 1000"  # :dtr --plot-metric-extra-strs :::::v3-100-10 --dont-plot-extra-str
...
# echo $bin --label vary-context-dependence-v1 --n-replicates 5 --n-sim-events-per-proc 50 --carry-cap-list 350 --obs-times-list 100:350 --n-sim-seqs-per-gen-list 100 --context-depend-list 0:1 --lb-tau-list 0.0025 $common --final-plot-xvar obs-times  --n-sub-procs 10  # --pvks-to-plot 0 (and 1), do 'em separately
# common="--only-csv-plots --n-max-procs 25"
common="$common --base-outdir /fh/local/dralph/partis/tree-metrics"
echo $bin --label among-family-variation-v0 --n-replicates 30 --n-sim-events-per-proc 10 --carry-cap-list 1000 --obs-times-list 250 --n-sim-seqs-per-gen-list 0 --lb-tau-list 0.0025 \
     --parameter-variances-list n-sim-seqs-per-generation,15..15:n-sim-seqs-per-generation,50..50:n-sim-seqs-per-generation,250..250:n-sim-seqs-per-generation,25..50..75:n-sim-seqs-per-generation,15..50..150..500 \
     --final-plot-xvar parameter-variances $common
...
# # and use dtr-train-v3 below with --iseed 1 (i think this was either made or at least get-tree-metrics was run with n max queries?)
# # plus also other dtr training samples with n max queries set to 150k or so
