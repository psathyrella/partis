...
final_outpath = '%s%s' % (args.outdir, '' if args.paired_loci else '/partition.yaml')
...
def run_cmd(action):
    ...
    if action in ['annotate', 'get-selection-metrics'] and '--paired-outdir' not in cmd:
        cmd += ' --%s %s' % ('paired-outdir' if args.paired_loci else 'outfname', final_outpath)
    if action == 'get-selection-metrics':
        cmd += ' --treefname %s --plotdir %s/selection-metrics/plots --debug 1' % (args.treefname, args.outdir) #  --queries-to-include-fname %s, args.seqfname)# TODO wtf was qti set here?
