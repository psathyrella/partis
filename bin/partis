...
parent_parser.add_argument('--debug', type=int, default=0, choices=[0, 1, 2])
...
parent_parser.add_argument('--is-data', action='store_true', help='deprecated!')
...
parent_parser.add_argument('--parameter-dir', required=True, help='Directory to/from which to write/read sample-specific parameters')
...
parent_parser.add_argument('--alignment-dir', default=os.getcwd() + '/data/fam-alignments')  # TODO remove this
parent_parser.add_argument('--outfname')
parent_parser.add_argument('--plotdir', help='Base directory to which to write plots (no plots are written if this isn\'t set)')
parent_parser.add_argument('--ighutil-dir', default=os.getenv('HOME') + '/.local', help='Path to vdjalign executable. The default is where \'pip install --user\' typically puts things')
parent_parser.add_argument('--workdir', help='Temporary working directory (see also <no-clean>)')
parent_parser.add_argument('--persistent-cachefname')
parent_parser.add_argument('--abbreviate', action='store_true', help='abbreviate sequence ids')
parent_parser.add_argument('--n-procs', default='1', help='Max/initial number of processes over which to parallelize (Can be colon-separated list: first number is procs for hmm, second (should be smaller) is procs for smith-waterman, hamming, etc.)')
parent_parser.add_argument('--n-max-procs', default=250, help='never allow more processes than this')
...
parent_parser.add_argument('--n-sets', type=int, default=1, help='Run on sets of sequences of size <n> (i.e. \"k-hmm\")')
parent_parser.add_argument('--all-combinations', action='store_true', help='Run algorithm on *all* possible combinations of the input queries of length <n-sets> (otherwise we run on sequential sets of <n-sets> in the input file).')
...
parent_parser.add_argument('--dont-rescale-emissions', action='store_true')
...
subargs['cache-parameters'].append({'name' : '--min-observations-to-write', 'kwargs' : {'type' : int, 'default' : 20, 'help' : 'For hmmwriter.py, if we see a gene version fewer times than this, we sum over other alleles, or other versions, etc. (see hmmwriter)'}})
# subargs['cache-parameters'].append({'--use_mean_at_boundaries', 'action' : 'store_true', 'help' : 'see note in hmmwriter'}})
subargs['run-viterbi'].append({'name' : '--presto-output', 'kwargs' : {'action' : 'store_true', 'help' : 'write output file in presto format'}})
subargs['partition'].append({'name' : '--annotation-clustering', 'kwargs' : {'help' : 'Perform annotation-based clustering from Vollmers paper'}})
...
subargs['partition'].append({'name' : '--naive-hamming-bounds', 'kwargs' : {}})
...
subargs['partition'].append({'name' : '--naive-vsearch', 'kwargs' : {'action' : 'store_true'}})
subargs['partition'].append({'name' : '--naive-swarm', 'kwargs' : {'action' : 'store_true'}})
subargs['partition'].append({'name' : '--synthetic-distance-based-partition', 'kwargs' : {'action' : 'store_true'}})
...
subargs['partition'].append({'name' : '--cache-naive-hfracs', 'kwargs' : {'action' : 'store_true'}})
subargs['partition'].append({'name' : '--n-precache-procs', 'kwargs' : {'type' : int, 'help' : 'Number of processes to use when precaching naive sequences. Only really useful for testing.'}})
subargs['partition'].append({'name' : '--seed-unique-id', 'kwargs' : {'help' : 'only look for sequences that are clonally related to this unique id'}})
...
subargs['partition'].append({'name' : '--n-partitions-to-write', 'kwargs' : {'type' : int, 'default' : 100, 'help' : ''}})
...
subargs['simulate'].append({'name' : '--n-trees', 'kwargs' : {'type' : int, 'default' : 500, 'help' : 'Number of trees to generate'}})
