    ...
    def prep_inputs():
        ...
        if not args.guess_pairing_info and not args.no_pairing_info and not has_input_meta_pair_info:
            if utils.getsuffix(args.infname) == '.yaml':
                ...
                utils.simplerun('%s/parse-output.py %s %s' % (os.path.dirname(os.path.realpath(__file__)), args.infname, new_fn), extra_str='    ')
                args.infname = new_fn
...
parent_args.append({'name' : '--phylo-naive-inference', 'kwargs' : {'type' : int, 'help' : 'when inferring trees, replace the original inferred naive seq with the inferred root from the phylo program. When passing the naive seq/root to the phylo program, we replace uncertain positions (e.g. non-templated insertions) with Ns in the naive sequence before passing to tree inference. The phylo-inferred naive sequence is added to the annotation as an observed sequence named \'phylo-naive\'.'}})
