...
# ----------------------------------------------------------------------------------------
def run_igdiscover(infname, outfname):
    ...
    cmds += ['igdiscover init --db %s/db --reads %s igdwork' % (args.workdir, args.infname)]
    cmds += ['cp %s igdwork/' % args.yamlfname]
    cmds += ['cd %s/' % args.workdir + '/igdwork']
    cmds += ['igdiscover run']
    utils.simplerun(';\n'.join(cmds), shell=True, print_time='igdiscover')

# ----------------------------------------------------------------------------------------
parser = argparse.ArgumentParser()
parser.add_argument('--infname', required=True)
parser.add_argument('--outfname', required=True)
parser.add_argument('--yamlfname', default='test/igdiscover.yaml')
...
run_igdiscover(args.infname, args.outfname)
