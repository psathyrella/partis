    ...
    if action == 'get-lb-bounds':
        ...
        if action == 'plot':
            for metric, estr in zip(args.plot_metrics, args.plot_metric_extra_strs):
                ...
                for ptvar, ptlabel, use_relative_affy in cfg_list:
                    ...
                    for cgroup in treeutils.cgroups:
                        ...
                        arglist, kwargs = (args, action, metric, pchoice, cgroup, ptvar, ptlabel, args.final_plot_xvar, get_tm_fname), {'use_relative_affy' : use_relative_affy, 'metric_extra_str' : estr, 'debug' : args.debug}
                        if args.test:
            ...
            for ptvar, ptlabel, use_relative_affy in cfg_list:
                ...
                for cgroup in treeutils.cgroups:
                    print '  ', cgroup
                    scanplot.make_plots(args, action, None, pchoice, cgroup, ptvar, ptlabel, args.final_plot_xvar, get_tm_fname, use_relative_affy=use_relative_affy)
