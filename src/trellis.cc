...
void trellis::viterbi() {
  ...
  for(size_t st = 0; st < hmm->n_states(); ++st) {
    if ((*initial_to)[st]) {  //if the bitset is set (meaning there is a transition to this state), calculate the viterbi
      viterbi_temp = hmm->state(st)->emission_score(*seqs,0) + init->transition_score(st);
  ...
  for(size_t position=1; position<seqs->GetSequenceLength(); ++position) {
	...
	if ((*scoring_previous)[st_previous] != -INFINITY) {
	  viterbi_temp = hmm->state(st_previous)->transition_score(st_current) + emission + (*scoring_previous)[st_previous];
  ...
  for(size_t st_previous = 0; st_previous < hmm->n_states() ;++st_previous) {
    if ((*scoring_previous)[st_previous] > -INFINITY) {
      viterbi_temp = (*scoring_previous)[st_previous] + hmm->state(st_previous)->getEndTrans();
...
void trellis::forward() {
  ...
  for(size_t st=0; st<hmm->n_states(); ++st) {
    if ((*initial_to)[st]) {  // if the bitset is set (meaning there is a transition to this state), calculate the viterbi
      double emscore = hmm->state(st)->emission_score(*seqs, 0);
  ...
  for (size_t position=1; position<seqs->GetSequenceLength(); ++position) {
	...
	if ((*scoring_previous)[previous] != -INFINITY) {
	  forward_temp = (*scoring_previous)[previous] + emission + hmm->state(previous)->transition_score(st_current);
  ...
  for(size_t st_previous=0; st_previous<hmm->n_states(); ++st_previous) {
    if ((*scoring_previous)[st_previous] != -INFINITY) {
      forward_temp = (*scoring_previous)[st_previous] + hmm->state(st_previous)->getEndTrans();
      if (forward_temp > -INFINITY) {
