...
void options::parse_commandline(int argc, const char * argv[]){
  for(int iopt=1; iopt<argc; iopt++) {
	...
	case OPT_INT:
	  ...
	  } else if (std::string(argv[iopt+1]).substr(0,2) == "--") {
	    if (!opts[tag]->default_value) {
	      std::cerr << "Command:\t" << tag <<" is missing a secondary command line argument\n";
	    }
	  } else {
	    //Probably need to check to make sure that the argument is a number;
	    opts[tag]->data.i = atoi(argv[++iopt]);
	  }
	  break;
	case OPT_DOUBLE:
	  if (iopt+1 == argc) {
	    ...
	    opts[tag]->data.d = atof(argv[++iopt]);
	  }
	  break;
	case OPT_STRING:
	  ...
	  if (iopt+1 == argc) {
	    ...
	    secondary = argv[iopt+1];
	  }
	  if (secondary.substr(0,2) == "--") {
	    if (!opts[tag]->default_value) {
	      opts[tag]->str="";
	    }
	  } else {
