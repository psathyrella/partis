...
#        with bz2.BZ2File(fname) as infile:
    ...
    def add_mutants(self, reco_event):
        ...
        n_tries = 0
        while util.are_conserved_codons_screwed_up(reco_event):  # keep generating trees until we get one that hasn't screwed up the conserved codons
            reco_event.final_seqs = []  # if not the first try, get rid of the last patch of seqs
            n_tries += 1
            leaf_seq_fname = tmpdir + '/leaf-seqs.fa'  # also in bpp-test.sh. Put it in only one place!
            call('./bin/run-bpp.sh ' + reco_seq_fname + ' ' + leaf_seq_fname + '>/dev/null', shell=True)  # write leaf node mutants to file (see .sh for details)
        ...
        print '    needed %d tries to mutatate' % n_tries
