...
def get_cmd(action, base_args, varnames, vlists, vstrs, all_simdirs=None):
    ...
    if action in ['simu', 'check-dl', 'merge-simu']:
        ...
        cmd = './projects/replay-plot.py --simu-like-dir %s --outdir %s --plot-labels data-d20:simu:simu-iqtree --n-max-simu-trees 85 --normalize' % (os.path.dirname(ofname(args, varnames, vstrs, 'check-dl' if action=='replay-plot-ckdl' else 'simu')), odr)
    elif action in ['dl-infer', 'dl-infer-merged', 'group-expts']:
