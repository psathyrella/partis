    ...
    def make_single_tree(self, partitions, annotations, uid_set, naive_seq_name, get_fasttrees=False, n_max_cons_seqs=10, debug=False):
        ...
        for lnode in dtree.leaf_node_iter():
            ...
            if get_fasttrees and len(lnode.uids) > 2:
                ...
                subtree = treeutils.run_tree_inference('fasttree', seqfos=seqfos, suppress_internal_node_taxa=True)  # note that the fasttree distances get ignored below (no idea if they'd be better than what we set down there, but they probably wouldn't be consistent, so I'd rather ignore them)
