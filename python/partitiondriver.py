...
class PartitionDriver(object):
    ...
    def calc_tree_metrics(self, annotation_dict, annotation_list=None, cpath=None):
        ...
        treeutils.calculate_tree_metrics(self.args.selection_metrics_to_calculate, annotation_dict, self.args.lb_tau, lbr_tau_factor=self.args.lbr_tau_factor, cpath=cpath, reco_info=self.reco_info, treefname=self.args.treefname,
                                         ...
                                         only_csv=self.args.only_csv_plots, min_cluster_size=self.args.min_selection_metric_cluster_size, dtr_path=self.args.dtr_path, include_relative_affy_plots=self.args.include_relative_affy_plots,
                                         plot_cfg=self.args.selection_metric_plot_cfg, cluster_indices=self.args.cluster_indices, outfname=self.args.selection_metric_fname, only_use_best_partition=self.args.only_print_best_partition, glfo=self.glfo,
                                         queries_to_include=self.args.queries_to_include, label_tree_nodes=self.args.label_tree_nodes, ignore_existing_internal_node_labels=self.args.ignore_internal_node_names, debug=self.args.debug)
    ...
    def process_annotation_output(self, annotation_list, hmm_failures, count_parameters=False, parameter_out_dir=None, print_annotations=False):
        ...
        if self.args.plotdir is not None and self.args.plot_tree_mut_stats:
            import plotting
