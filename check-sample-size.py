...
plot_cmd = './python/compare.py --plotdirs ' + ':'.join(plotdirs)
plot_cmd += ' --names ' + ':'.join(names) + ' --outdir ' + baseplotdir + '/tmp'
plot_cmd += ' --leaves-per-tree ' + ':'.join(['5' for _ in range(len(names))]) + ' --colors ' + ':'.join([str(c) for c in colors])
plot_cmd += ' --linestyles ' + ':'.join([str(l) for l in linestyles])
plot_cmd += ' --normalize --markersize 0 --no-errors'
