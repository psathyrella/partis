...
class PartitionDriver(object):
    ...
    def write_output(self, annotation_list, hmm_failures, cpath=None, dont_write_failed_queries=False, write_sw=False, outfname=None, extra_headers=None):
        ...
        if self.args.extra_annotation_columns is not None and 'linearham-info' in self.args.extra_annotation_columns:  # it would be nice to do this in utils.add_extra_column(), but it requires sw info, which would then have to be passed through all the output infrastructure
            utils.add_linearham_info(self.sw_info, annotation_list, self.glfo, min_cluster_size=3)  # NOTE this only controls whether we write linearham bounds into the annotation (which is cheap); it does *not* control how many clusters linearham actually gets run on (that's linearham-run.py --min-cluster-size, default 5, and in the linearham scons file parse_cluster.py picks a single cluster anyway). So the only cost of a low threshold here is output size on files with lots of small clusters, which is why there's still a threshold at all rather than None. Not worth propagating --cluster-indices or --seed/lineage-unique-ids in here, and --min-selection-metric-cluster-size is the wrong knob (its default is 10, which would make this worse)
