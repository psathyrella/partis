...
def process(args):
    ...
    if (getattr(args, 'hybrid', False) or getattr(args, 'refine', False)) and not getattr(args, 'disjoint_groups', False):
        raise Exception('--hybrid/--refine require --disjoint-groups')
    ...
    if os.path.exists(args.workdir) and args.action not in ['create-disjoint-groups', 'assemble-groups', 'create-hybrid-jobs', 'run-hybrid-jobs', 'assemble-hybrid', 'run-refine-jobs']:
