...
class Tester(object):
    ...
    def __init__(self):
        ...
        self.datafname = '%s/test/mishmash.fa' % utils.get_partis_dir()  # some data from adaptive, chaim, and vollmers
        ...
        self.paired_datafname = '%s/test/paired-data/all-seqs.fa' % utils.get_partis_dir()
        self.input_metafname = '%s/test/input-meta.yaml' % utils.get_partis_dir()
        ...
        def add_inference_tests(input_stype):  # if input_stype is 'ref', infer on old simulation and parameters, if it's 'new' use the new ones
            ...
            if args.paired:
                self.tests['subset-partition-%s-simu'%input_stype] = {'extras' : ['--max-ccf-fail-frac', '0.25']} # '--biggest-logprob-cluster-to-calculate', '5', '--biggest-naive-seq-cluster-to-calculate', '5',
