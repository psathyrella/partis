...
for tst in $tests; do
    ...
    $cmd --cache_parameters --seqfile test/every-hundredth-data.tsv.bz2 --is_data --skip_unproductive --parameter_dir caches/$tst --plotdir $www/partis/$tst --n_max_queries 10000 $common_args
    simu_file=caches/recombinator/$tst/simu.csv 
    $cmd --simulate --parameter_dir caches/$tst/hmm_parameters --n_max_queries 2000 --outfname $simu_file $common_args
