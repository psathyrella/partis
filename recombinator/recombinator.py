...
class Recombinator(object):
    """ Simulates the process of VDJ recombination """
    def __init__(self, datadir, parameter_dir, human, naivety, only_genes='', total_length_from_right=0, apply_shm=True):  # yes, with a y!
        ...
        if self.apply_shm:
            with opener('r')(self.parameter_dir + '/human-beings/' + self.human + '/' + self.naivety + '/gtr.txt') as gtrfile:  # read gtr parameters
            ...
            with opener('r')(self.parameter_dir + '/human-beings/' + self.human + '/' + self.naivety + '/trees.tre') as treefile:  # read in the trees that were generated by tree-gen.r
                self.trees = treefile.readlines()

        if only_genes != '':
            print '    restricting to: %s' % only_genes
            self.restrict_gene_choices(only_genes)
        print '    reading version freqs from %s' % (self.parameter_dir + '/human-beings/' + self.human + '/' + self.naivety + '/' + utils.all_column_fname)
        self.read_vdj_version_freqs(self.parameter_dir + '/human-beings/' + self.human + '/' + self.naivety + '/' + utils.all_column_fname)
...
#        self.write_final_vdj(reco_event)
    ...
    def write_mute_freqs(self, region, gene_name, seq, reco_event, reco_seq_fname):
        ...
        if 'insert' in gene_name:
            ...
            mutefname = self.parameter_dir + '/human-beings/' + self.human + '/' + self.naivety + '/mute-freqs/' + utils.sanitize_name(gene_name) + '.csv'
